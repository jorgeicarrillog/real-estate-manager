<script>
    import { InertiaLink, page } from '@inertiajs/inertia-svelte';
    import Helmet from '@/Shared/Helmet.svelte';
    import Layout from '@/Shared/Layout.svelte';
    import Icon from '@/Shared/Icon.svelte';
    import SearchFilter from '@/Shared/SearchFilter.svelte';
    import Pagination from '@/Shared/Pagination.svelte';

    const route = window.route;

    $: data = $page.owners.data;
    $: links = $page.owners.links;
</script>

<Helmet title="Propietarios" />

<Layout>
    <div>
        <h1 class="mb-8 font-bold text-3xl">Propietarios</h1>

        <div class="mb-6 flex justify-between items-center">
            <SearchFilter />

            <InertiaLink class="btn-indigo" href={route('owners.create')}>
                <span>Crear</span>
                <span class="hidden md:inline"> Propietario</span>
            </InertiaLink>
        </div>

        <div class="bg-white rounded shadow overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                <thead>
                    <tr class="text-left font-bold">
                        <th class="px-6 pt-5 pb-4">Nombre</th>
                        <th class="px-6 pt-5 pb-4">Correo</th>
                        <th class="px-6 pt-5 pb-4">Propiedades</th>
                        <th class="px-6 pt-5 pb-4">Inmobiliaria</th>
                    </tr>
                </thead>
                <tbody>
                    {#if !data || data.length === 0}
                        <tr>
                            <td class="border-t px-6 py-4" colspan="4">
                                No owners found.
                            </td>
                        </tr>
                    {:else}
                        {#each data as { id, name, photo, email, properties_count, organization } (id)}
                            <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                                <td class="border-t">
                                    <InertiaLink
                                        href={route('owners.edit', id)}
                                        class="px-6 py-4 flex items-center focus:text-indigo-700"
                                    >
                                    {#if photo}
                                        <img
                                            src={photo}
                                            class="block w-5 h-5 rounded-full mr-2 -my-2"
                                            alt={name}
                                        />
                                    {/if}
                                    {name}
                                    </InertiaLink>
                                </td>

                                <td class="border-t">
                                    <InertiaLink
                                        tabindex="-1"
                                        href={route('owners.edit', id)}
                                        class="px-6 py-4 flex items-center focus:text-indigo"
                                    >
                                        {email}
                                    </InertiaLink>
                                </td>

                                <td class="border-t">
                                    <InertiaLink
                                        tabindex="-1"
                                        href={route('owners.edit', id)}
                                        class="px-6 py-4 flex items-center focus:text-indigo"
                                    >
                                        {properties_count}
                                    </InertiaLink>
                                </td>

                                <td class="border-t">
                                    <InertiaLink
                                        tabindex="-1"
                                        href={route('owners.edit', id)}
                                        class="px-6 py-4 flex items-center focus:text-indigo"
                                    >
                                        {organization}
                                    </InertiaLink>
                                </td>

                                <td class="border-t w-px">
                                    <InertiaLink
                                        tabindex="-1"
                                        href={route('owners.edit', id)}
                                        class="px-4 flex items-center"
                                    >
                                        <Icon
                                            name="cheveron-right"
                                            className="block w-6 h-6 text-gray-400 fill-current"
                                        />
                                    </InertiaLink>
                                </td>
                            </tr>
                        {/each}
                    {/if}
                </tbody>
            </table>
        </div>

        <Pagination links={links} />
    </div>
</Layout>
